...
def svm_test(clf, test_input, test_output):
    ...
    new_y_pred = []
    for elem in y_pred:
        new_y_pred.append(int(elem))
    new_y_true = []
    for elem in y_true:
        new_y_true.append(int(elem))
    resdict = {'pred':list(new_y_pred), 'true':list(new_y_true)}
    print(resdict)

    with open('res.json', 'w', encoding='utf8') as json_file:
        json.dump(resdict, json_file, ensure_ascii=False)
...
def trainIO(train_index, test_index):
    ...
    if config.use_target_text and not config.use_only_speaker:
