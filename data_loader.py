...
class DataLoader:
    ...
    def __init__(self, config, path_prefix):
        ...
        if config.use_bert and config.use_target_text and not config.modified:
        ...
        #
            ...
            with jsonlines.open(self.SBERT_EBBEDDINGS) as reader:
                for obj in reader:
                    text_bert_embeddings = list(obj)

        elif config.use_sbert_token and config.use_target_text:
            print('modified sbert_token')
            with jsonlines.open(self.SBERT_EBBEDDINGS) as reader:
                for obj in reader:
                    text_bert_embeddings = list(obj)

        elif config.use_universal and config.use_target_text:
            print('modified universal embedding')
...
class DataHelper:
    ...
    def __init__(self, train_input, train_output, test_input, test_output, config, dataLoader):
        ...
        if not config.use_only_speaker:
